...
class ALFWorldMemory(BaseMemory):
    """
    A custom Memory implementation tailored for the ALFWorld environment.

    Unlike ``SimpleMemory``, this class formats each history step in a
    structured, human-readable block that separates the observation from
    the action.  Example output for one step::

        Step 1:
          Observation: You are in the kitchen. ...
          Action: go to fridge 1

    The cleaner layout helps the language model distinguish past
    observations from past actions when the history grows long.
    """
    ...
    def store(self, record: Dict[str, List[Any]]):
        ...
        Args:
            record (Dict[str, List[Any]]):
                A dictionary where each key corresponds to a type of data
                (e.g., ``'text_obs'``, ``'action'``), and each value is a
                list of length ``batch_size``, containing the data for each
                environment.
        """
        if self.keys is None:
            self.keys = list(record.keys())
    ...
    def fetch(
        ...
        instance using an ALFWorld-specific structured layout.
        ...
        Returns:
            memory_contexts : List[str]
                A list of formatted history strings, one per environment.
            valid_lengths : List[int]
                A list of the actual number of valid history steps per
                environment.
        """
        memory_contexts, valid_lengths = [], []
